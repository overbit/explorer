...
const DropdownHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
    ({ className, ...props }, ref) => (
        <div
            ref={ref}
            className={cn('mb-0 whitespace-nowrap px-6 py-2 text-dk-sm text-dk-gray-500', className)}
            {...props}
        />
    ),
);
    ...
    ({ className, asChild = false, onClick, ...props }, ref) => {
        ...
        return (
            <Comp
                ref={ref}
                className={cn(
                    'clear-both block w-full whitespace-nowrap border-0 bg-transparent px-6 py-1.5 text-left font-normal text-dark-muted-foreground no-underline hover:text-white focus:text-white active:text-white disabled:pointer-events-none disabled:text-dk-gray-500 [&.active]:text-white [&.disabled]:pointer-events-none [&.disabled]:text-dk-gray-500',
                    className,
                )}
                onClick={event => {
                    onClick?.(event);
