...
function ColorBox({ color }: { color: string }) {
    return <div aria-hidden style={{ ...BOX, background: color, border: '1px solid #1a1a1a' }} />;
}
            ...
            {Object.entries(extend.borderRadius).map(([name, value]) => (
                <TokenRow
                    ...
                    preview={
                        <div
                            aria-hidden
                            style={{ ...BOX, background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: value }}
                        />
                    }
                    label={name}
                    value={value}
    ...
    render: () => (
        <Grid>
            {Object.entries(extend.boxShadow).map(([name, value]) => (
                <TokenRow
                    key={name}
                    preview={<div aria-hidden style={{ ...BOX, background: '#0a0a0a', boxShadow: value }} />}
                    label={name}
                    value={value}
                />
            ))}
