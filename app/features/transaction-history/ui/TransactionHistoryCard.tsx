...
function TransactionRow({
    ...
    const { signature, slot, blockTime, status, memo } = row;
        ...
        <>
            ...
            >
                <div className={cn('flex flex-col text-sm lg:hidden', ROW_PADDING)}>
                    ...
                    <MobileField label="Memo">{memo ? <MemoCell memo={memo} /> : '---'}</MobileField>
                ...
                <div className={cn('hidden items-baseline gap-4 lg:grid', ROW_PADDING, historyGridCols(hasTimestamps))}>
                    ...
                    <div className="min-w-0 text-sm">{memo ? <MemoCell memo={memo} /> : '---'}</div>
